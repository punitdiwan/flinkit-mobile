...
const Product_Details = ({ route }: any) => {
  const { name, imgUrl, id } = route.params;
    ...
    <View style={{ padding: 15 }}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', gap: 12 }}>
          ...
          {quantity === 0 ? <TouchableOpacity onPress={() => increaseCardQuantity(id)} style={{ paddingHorizontal: 20, paddingVertical: 10, backgroundColor: '#316e41', borderRadius: 25 }}>
          ...
          >
            ...
            <Text onPress={() => decreaseCardQuantity(id)} style={{ fontSize: 30, fontWeight: '500', color: "#ffffff", height: '100%' }}>-</Text>
            <Text style={{ fontSize: 15, fontWeight: '500', color: '#ffffff' }}>{quantity}</Text>
            <Text onPress={() => increaseCardQuantity(id)} style={{ fontSize: 30, fontWeight: '500', color: '#ffffff', height: '100%' }}>+</Text>
          </View>}
